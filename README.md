...
### 🧩 2048 - AI智能版
- 🤖 **多种AI算法** - Expectimax、贪心、角落策略、随机算法、MCTS（蒙特卡洛树搜索）
- 🛠️ **MCTS参数可视化调节** - 前端滑块实时调整蛇形权重、合并潜力、角落奖励、UCB1探索参数、模拟次数、深度等
- 🌲 **MCTS树持久化** - 每步复用历史搜索树，提升智能和效率
- ✂️ **UCT剪枝预留** - 支持top-k/阈值剪枝参数，后端可灵活开启
- 🚦 **高效串行rollout** - 优化模拟效率，兼容所有环境
...
### 2. MCTS蒙特卡洛树搜索 ⭐⭐⭐⭐⭐
**高级AI算法，支持全参数可调与树持久化**：
- 支持前端参数面板实时调节（蛇形权重、合并潜力、角落奖励、UCB1探索、模拟次数、深度等）
- 每步复用历史搜索树（树持久化），大幅提升智能和效率
- 支持UCT剪枝（top-k/阈值），可在后端开启
- 高效串行rollout，兼容所有平台
- **适合AI实验、参数对比和高级玩法**
...
- 进入2048页面，右侧可见"MCTS参数调节"面板，拖动滑块即可实时调整AI参数。
- 每次点击"AI走一步"或"自动游戏"，参数会自动传递到后端，动态生效。
- 后端也可通过 `MCTSAI` 构造参数自定义（如 `uct_top_k`、`ucb1_threshold`）。
- 支持自动化实验、参数对比、AI风格切换。
