...
from .config import EXPECTIMAX_CONFIG
import sys
import os
...
class ExpectimaxAI(BaseAI):
    """基于Expectimax算法的2048 AI求解器"""
    
    def __init__(self, max_depth: int = None):
        super().__init__("Expectimax AI")
        # 使用配置文件中的默认值
        self.max_depth = max_depth if max_depth is not None else EXPECTIMAX_CONFIG['max_depth']
        self.weights = EXPECTIMAX_CONFIG['weights'].copy()
